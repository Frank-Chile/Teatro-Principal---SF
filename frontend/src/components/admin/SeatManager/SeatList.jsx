...
function SeatList() {
    ...
    const [editingButaca, setEditingButaca] = useState(null);
    ...
    const fetchButacasYFuncion = useCallback(async () => {
        ...
        try {
            ...
            if (err.response?.data?.detail && err.response.data.detail.includes("Función no encontrada")) {
    ...
    const handleDeleteButaca = async (butacaId) => {
        if (window.confirm('¿Estás seguro de que quieres eliminar esta butaca?')) {
            try {
                ...
                alert(err.response?.data?.detail || err.message || 'Error al eliminar la butaca.');
            }
        }
    };
                    ...
                    <table style={{ width: '100%', marginTop: '10px', borderCollapse: 'collapse' }}>
                        <thead>
                            <tr>
                                ...
                                <th style={tableHeaderStyle}>Comprado por</th>
                        ...
                        <tbody>
                            {butacas.map((butaca) => (
                                <tr key={butaca.id}>
                                    ...
                                    <td style={tableCellStyle}>${(butaca.precio_base_calculado || 0).toFixed(2)}</td>
                                    <td style={tableCellStyle}>{butaca.vendida ? 'Sí' : 'No'}</td>
                                    <td style={tableCellStyle}>{butaca.comprador_username || '---'}</td>
                                    <td style={tableCellStyle}>
