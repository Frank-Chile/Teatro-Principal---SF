...
import './SeatMapView.css'; // Asegúrate de que este archivo CSS esté importado
    ...
    const handleProceedToCheckout = () => {
        ...
        comprarButacasClient(funcionId, Array.from(selectedButacas))
            .then(compraData => {
                ...
                Función: ${funcion?.nombre_obra}
                Butacas Compradas: ${compraData.butacas_compradas.map(b => `F${b.fila}N${b.numero}`).join(', ')}
                Total Pagado: $${compraData.total_pagado.toFixed(2)}
                Ticket: ${compraData.mensaje}`);
                
                fetchFuncionYButacas();
                setSelectedButacas(new Set());
            })
            .catch(err => {
                setPurchaseError(err.response?.data?.detail || err.message || "Error durante el proceso de compra.");
            })
            .finally(() => {
                    ...
                    <div key={fila} className="seat-row">
                        ...
                        {asientos.map(butaca => (
                            <button
                                ...
                                `} // La clase 'selected' se aplica aquí condicionalmente
            ...
            {selectedButacas.size > 0 && (
                <div className="checkout-summary">
                    ...
                    <button onClick={handleProceedToCheckout} disabled={isPurchasing}>
                        {isPurchasing ? 'Procesando...' : 'Continuar con la Compra (Simulación)'}
                    </button>
                    {purchaseError && <p className="error-message" style={{marginTop: '10px'}}>{purchaseError}</p>}
                </div>
            )}
